export async function onRequest(context) {
  ...
  if (request.method === "GET") {
    const { results } = await env.DB.prepare(
      "SELECT video_id as id, title, channel, thumbnail FROM favorites WHERE user_id = ? ORDER BY created_at DESC"
    )
      .bind(userId)
      .all();
  ...
  if (request.method === "POST") {
    ...
    await env.DB.prepare(
      `INSERT OR IGNORE INTO favorites (user_id, video_id, title, channel, thumbnail)
       VALUES (?, ?, ?, ?, ?)`
    )
